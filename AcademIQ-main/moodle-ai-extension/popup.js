...
const API_BASE_STORAGE = "academiq_api_base";
...
const refs = {
    ...
    studentIdLine: document.getElementById("studentIdLine"),
    apiBaseUrl: document.getElementById("apiBaseUrl"),
    syncApiBtn: document.getElementById("syncApiBtn"),
    persistMongo: document.getElementById("persistMongo"),
...
const renderStudentId = (data) => {
    const el = refs.studentIdLine;
    if (!el) return;
    const sid = data?.student?.student_id;
    if (sid) {
        el.textContent = `Student ID (for web sign-in): ${sid}`;
        el.classList.remove("missing");
    } else {
        el.textContent =
            "Student ID: open any page on your Moodle site in Chrome, then Refresh here.";
...
const loadApiBase = () =>
    new Promise((resolve) => {
        chrome.storage.local.get(API_BASE_STORAGE, (res) => {
            const v = res[API_BASE_STORAGE];
            resolve(typeof v === "string" && v.trim() ? v.trim() : DEFAULT_API_BASE);
        });
    });

const setSyncStatus = (text, kind) => {
    if (!refs.syncStatus) return;
    refs.syncStatus.textContent = text;
    refs.syncStatus.classList.remove("ok", "err");
    if (kind === "ok") refs.syncStatus.classList.add("ok");
    if (kind === "err") refs.syncStatus.classList.add("err");
};

refs.syncApiBtn?.addEventListener("click", async () => {
    const data = await getStorageData();
    if (!data) {
        setSyncStatus("No Moodle data to sync. Browse Moodle first.", "err");
        return;
    }
    const base = (refs.apiBaseUrl?.value || DEFAULT_API_BASE).trim().replace(/\/$/, "");
    if (!/^https?:\/\//i.test(base)) {
        setSyncStatus("Enter a valid API URL (http://127.0.0.1:8000).", "err");
        return;
    }
    chrome.storage.local.set({ [API_BASE_STORAGE]: base });
    const payload = sanitizePayload(data);
    const persist = Boolean(refs.persistMongo?.checked);
    const url = `${base}/analyze?persist_mongo=${persist ? "true" : "false"}`;
    setSyncStatus("Syncing…", "");
    refs.syncApiBtn.disabled = true;
    try {
        ...
        if (!res.ok) {
            const detail = json.detail || json.error || text || res.status;
            setSyncStatus(`Error ${res.status}: ${typeof detail === "string" ? detail : JSON.stringify(detail)}`, "err");
            return;
        }
        const line1 = `Last sync (API): Risk ${json.risk_cluster} — ${json.recommendation || ""}`;
        const line2 = json.warning ? `\n${json.warning}` : "";
        const line3 = json.persisted_mongo ? "\nSaved to MongoDB." : "";
        setSyncStatus(`${line1}${line2}${line3}`, "ok");
    } catch (e) {
        setSyncStatus(`Network error: ${e.message || e}. Is the API running?`, "err");
    } finally {
        refs.syncApiBtn.disabled = false;
    }
});
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (refs.apiBaseUrl) refs.apiBaseUrl.value = apiBase;
    refreshData();
});
